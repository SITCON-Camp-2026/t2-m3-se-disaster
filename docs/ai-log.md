...
| 16:09 | Release 02 | 觀測者新增資料與本地持久化流程 | 建議把觀測者新增資料設計成本機未整理草稿，用本地持久化保存，但一律維持未查核與需要人工確認                             | 採用為流程草稿   | 符合前端-only 與不新增 fixture 的限制；本機資料不可假裝成正式資料，也不可保存真實個資或真實災情       | `docs/flow.md`, `docs/decisions.md`                                         |
